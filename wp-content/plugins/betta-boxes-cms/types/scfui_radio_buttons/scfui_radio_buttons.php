<?php
class scfui_radio_buttons extends scfui_type {
	function scfui_radio_buttons() {
	}
	...
	public function showField($post, $field) {
		...
		foreach ((array)$extra['options'] as $key=>$val) {
			echo '<input type="radio" name="scfui['.$field['slug'].']" value="'.htmlspecialchars($val).'" '.($value == $val ? 'checked="checked"' : '').' /> '.htmlspecialchars($val).'<br />';
		}
		echo '</p>';
	}
